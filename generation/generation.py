...
def uddia(prompts: pd.Series,
    ...
    # We are printing each generation to the output file as soon as it is produced
    prompts = prompts[num_cached_generations:]
    for pp in prompts:
        ret, records = uddia_func(pp)

        for record in records:
            with records_file.open('a') as f:
                print(json.dumps(record), file=f)

        for generation in ret:
